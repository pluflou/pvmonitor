...
class PvMonitor:
    ...
    def __init__(
        ...
        pv_groups: Optional[List[List[int]]] = None, 
        ...
        self.pv_groups = pv_groups or [[i] for i in range(len(pv_list))]
        self.group_titles = group_titles or pv_list
    ...
    def setup(self):
        ...
        n_plots = len(self.pv_groups)
        n_rows = math.ceil(n_plots / self.n_cols)
        self.fig, self.axes = plt.subplots(
            n_rows, self.n_cols, squeeze=False,
            constrained_layout=True, 
            figsize=(self.n_cols * 3.8, n_rows * 4.9),
            dpi=130
        )
        
        self.fig.canvas.manager.set_window_title("")
        self.fig.suptitle("LCLS Copper Injector Surrogate Model \n Live Predictions \n\n")

        class PacificFormatter(mdates.DateFormatter):
            def __init__(self, fmt, tz):
                super().__init__(fmt, tz=tz)

        colors = sns.color_palette("husl", len(self.pv_list))
    
        for i, (group, title) in enumerate(zip(self.pv_groups, self.group_titles)):
            a = self.axes.flatten()[i]
            a.set_title(title, fontsize=10)
            a.grid(True, alpha=0.25)
            a.xaxis.set_major_formatter(PacificFormatter("%m/%d %H:%M", tz=self.timezone))
            a.xaxis_date()
            ...
            # Create one line per PV in this group
            for pv_idx in group:
                pv_name = self.pv_list[pv_idx]
                (ln,) = a.plot([], [], "-", lw=1.2, label=pv_name)
                self.lines[pv_name] = (a, ln)

            a.legend(loc="best", fontsize=6.5)

        for j in range(n_plots, n_rows * self.n_cols):
